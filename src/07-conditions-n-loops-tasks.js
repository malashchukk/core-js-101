...
function getFactorial(n) {
  ...
  return res;
  // return arr.reduce((prev, curr) => {console.log(prev, curr); return (curr+1)*prev}, 1)
  // throw new Error('Not implemented');
}
...
function doRectanglesOverlap(rectA, rectB) {
  if ((rectB.left > (rectA.left + rectA.width))
   || (rectA.left > (rectB.left + rectB.width))
   || (rectA.top > (rectB.top + rectB.height))
   || (rectB.top > (rectA.top + rectA.height))) return false;
  // throw new Error('Not implemented');
  return true;
}
...
function isInsideCircle(circle, point) {
  ...
  if (((x ** 2) + (y ** 2) - circle.radius ** 2 < 0)) return true;
  return false;
  // throw new Error('Not implemented');
}
// console.log(0-10)
...
function getCommonDirectoryPath(pathes) {
  ...
  if (cmnInx === undefined) { return ''; }
  if (cmnInx === 0) { return first[0]; }
  return first.slice(0, cmnInx + 1);
  // throw new Error('Not implemented');
}
